...
- Install `KubeStash` in your cluster following the steps [here](/docs/v2026.2.16-rc.0/setup/README).
...
- You should be familiar with the following `KubeStash` concepts:
    - [BackupConfiguration](/docs/v2026.2.16-rc.0/concepts/crds/backupconfiguration/)
    - [BackupSession](/docs/v2026.2.16-rc.0/concepts/crds/backupsession/)
    - [RestoreSession](/docs/v2026.2.16-rc.0/concepts/crds/restoresession/)
    - [BackupStorage](/docs/v2026.2.16-rc.0/concepts/crds/backupstorage/)
    - [Snapshot](/docs/v2026.2.16-rc.0/concepts/crds/snapshot/)
    - [RetentionPolicy](/docs/v2026.2.16-rc.0/concepts/crds/retentionpolicy/)
...
In this guide, we'll use the [mountpoint-s3-csi driver](https://github.com/awslabs/mountpoint-s3-csi-driver), which is AWS officially maintained. 
...
For alternative installation methods, follow the official installation guide of the driver [here](https://github.com/awslabs/mountpoint-s3-csi-driver/blob/main/docs/INSTALL.md).

> **Note:** According to this issue [slow mountpoitn](https://github.com/awslabs/mountpoint-s3/issues/953), mountpoint-s3 is slower than other S3 CSI drivers.
> Here is some comparism of performance and read/write speed [link](https://github.com/yandex-cloud/geesefs/blob/master/bench/README.md#mdtest). So, use another S3 CSI driver if you need better performance.
...
We are going to use a `GCS` bucket. For this, we have to create a `Secret` with necessary credentials and a `BackupStorage` object. If you want to use a different backend, please read the respective backend configuration doc from [here](/docs/v2026.2.16-rc.0/guides/backends/overview/).

> For GCS backend, if the bucket does not exist, KubeStash needs `Storage Object Admin` role permissions to create the bucket. For more details, please check the following [guide](/docs/v2026.2.16-rc.0/guides/backends/gcs/).
...
Notice the `spec.usagePolicy` that allows referencing the `RetentionPolicy` from all namespaces.For more details on configuring it for specific namespaces, please refer to the following [RetentionPolicy usage policy](/docs/v2026.2.16-rc.0/concepts/crds/retentionpolicy/).
