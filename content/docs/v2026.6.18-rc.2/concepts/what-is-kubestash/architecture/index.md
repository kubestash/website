...
Backend is the storage where KubeStash stores backed up files. It can be a cloud storage like GCS bucket, AWS S3, Azure Blob Storage etc. or a Kubernetes persistent volume like [NFS](https://kubernetes.io/docs/concepts/storage/volumes/#nfs), [PersistentVolumeClaim](https://kubernetes.io/docs/concepts/storage/volumes/#persistentvolumeclaim), etc. To learn more about backend, please visit [here](/docs/v2026.6.18-rc.2/guides/backends/overview/).

### CronJob

When a user creates a [BackupConfiguration](/docs/v2026.6.18-rc.2/concepts/crds/backupconfiguration/) object, KubeStash creates a CronJob with the schedule specified in it. At each scheduled slot, this CronJob triggers a backup for the targeted workload.

### Backup Job

When a user creates a [BackupConfiguration](/docs/v2026.6.18-rc.2/concepts/crds/backupconfiguration/) object, KubeStash creates a job or multiple jobs (depending on target) to take backup at each trigger.

### Restore Job

When a user creates a [RestoreSession](/docs/v2026.6.18-rc.2/concepts/crds/restoresession/) object, KubeStash creates a job or multiple jobs (depending on target) to restore the target.
