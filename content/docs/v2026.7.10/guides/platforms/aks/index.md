...
- Install `KubeStash` in your cluster following the steps [here](/docs/v2026.7.10/setup/README). You need to provide some labels and annotations during installation, see [here](#prepare-kubestash-operator).
...
Notice the `spec.runtimeSettings`, here we have to provide the label `azure.workload.identity/use: "true"` in `podLabes` and have to provide the `serviceAccountName`. These values will be set in the cleaner job created by the KubeStash operator. To learn more about `spec.runtimeSettings`, visit [here](/docs/v2026.7.10/concepts/crds/backupstorage#runtimeSettings).
