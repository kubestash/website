...
- If you haven't read about how hooks work in KubeStash, please check it from [here](/docs/v2025.6.30/guides/hooks/overview/).
...
- [HookTemplate](/docs/v2025.6.30/concepts/crds/hooktemplate/)
...
We are going to store our backed up data into a GCS bucket. We have to create a Secret with necessary credentials and a BackupStorage crd to use this backend. If you want to use a different backend, please read the respective backend configuration doc from [here](/docs/v2025.6.30/guides/backends/overview/).
...
spec:
  ...
  usagePolicy:
    allowedNamespaces:
      from: All
```
Notice the `spec.usagePolicy` that allows referencing the `RetentionPolicy` from all namespaces. To allow specific namespaces, we can configure it accordingly by following [RetentionPolicy usage policy](/docs/v2025.6.30/concepts/crds/retentionpolicy/#retentionpolicy-spec).
